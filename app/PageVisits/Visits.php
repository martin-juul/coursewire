...
use App\PageVisits\Traits\{Lists, Periods, Record, Setters};
...
class Visits
{
    ...
    /**
     * @var \App\PageVisits\DataEngines\DataEngine
     */
     ...
     * @return self
     */
    public function by($subject): self
    {
     ...
     * @return bool
     */
    public function recordedIp(): bool
    {
    ...
    public function count()
    {
        ...
        return (int)($this->keys->instanceOfModel
     ...
     * @return integer time left in seconds
     */
    public function ipTimeLeft(): int
    {
    ...
    public function increment($inc = 1, $force = false, $ignore = []): bool
    {
        if ($force || (!$this->isCrawler() && !$this->recordedIp())) {
            ...
            foreach (['country', 'refer', 'periods', 'operatingSystem', 'language'] as $method) {
                if (!\in_array($method, $ignore, true)) {
     ...
     * @return bool
     */
    public function forceIncrement($inc = 1, $ignore = []): bool
    {
     ...
     * @return bool
     */
    public function decrement($dec = 1, $force = false, $ignore = []): bool
    {
     ...
     * @return bool
     */
    public function forceDecrement($dec = 1, $ignore = []): bool
    {
     ...
     */
    public function relation(): \Illuminate\Database\Eloquent\Relations\Relation
    {
        $prefix = $this->config['keys_prefix'] ?? $this->config['redis_keys_prefix'] ?? 'visits';

        return $this->subject->hasOne(PageVisit::class, 'secondary_key')
            ->where('primary_key', $prefix . ':' . $this->keys->visits);
    }
}
