...
class VisitsTestCase extends TestCase
{
    use RefreshDatabase;
    ...
    public function visits_by_user_lists()
    {
        ...
        $top_visits_overall = visits('Tests\Feature\PageVisits\Post')
            ->top(10)
            ->toArray();
        $this->assertEmpty($top_visits_overall);

        $top_visits = visits('Tests\Feature\PageVisits\Post')
            ->by($user)
            ->top(20)
            ->toArray();
    ...
    public function it_reset_counter()
    {
        ...
        $this->assertEquals(
            [2, 3],
            visits('Tests\Feature\PageVisits\Post')->top(5)->pluck('id')->toArray()
        );
    }
    ...
    public function it_shows_proper_tops_and_lows()
    {
        ...
        $this->assertEquals(
            ...
            visits('Tests\Feature\PageVisits\Post')->period('day')->top(10)->pluck('id')->toArray()
        );

        $this->assertEquals(
            Collection::make($arr)->sort()->keys()->take(10)->toArray(),
            visits('Tests\Feature\PageVisits\Post')->period('day')->low(11)->pluck('id')->toArray()
        );

        visits('Tests\Feature\PageVisits\Post')->period('day')->reset();

        $this->assertEquals(
            0,
            visits('Tests\Feature\PageVisits\Post')->period('day')->count()
        );
        // dd(visits('Tests\Feature\PageVisits\Post')->period('day')->top(10));

        $this->assertEmpty(
            visits('Tests\Feature\PageVisits\Post')->period('day')->top(10)
        );

        $this->assertNotEmpty(
            visits('Tests\Feature\PageVisits\Post')->top(10)
        );

        $this->assertEquals(
            Collection::make($arr)->sum(),
            visits('Tests\Feature\PageVisits\Post')->count()
        );
    }
    ...
    public function n_minus_1_bug()
    {
        ...
        $list = visits('Tests\Feature\PageVisits\Post')->top(5)->pluck('name');
    ...
    public function it_list_from_cache()
    {
        ...
        $fresh = visits('Tests\Feature\PageVisits\Post')->top()->pluck('name');

        $post5->update(['name' => 'changed']);

        $cached = visits('Tests\Feature\PageVisits\Post')->top()->pluck('name');

        $this->assertEquals($fresh->first(), $cached->first());

        $fresh2 = visits('Tests\Feature\PageVisits\Post')
            ->fresh()
            ->top()
            ->pluck('name');
    ...
    public function it_list_filtered_by_constraints()
    {
        ...
        $this->assertNotEquals(visits('Tests\Feature\PageVisits\Post')->top(5, ['name' => 'naji']), [$posts['naji']]);
    }
}
